...
import { useApi, useAPI, useAPIMutation } from '@/hooks/useApi'
...
function App() {
  ...
  // Legacy useAPI for backward compatibility demo
  const { data: legacyHealthData, error: legacyError, isLoading: legacyLoading } = useAPI('/health')
  
  // Legacy useAPIMutation for backward compatibility demo
  const { post: legacyPost } = useAPIMutation()
  ...
  const handleLegacyMutation = async () => {
    try {
      const result = await legacyPost('/api/test', { message: 'Hello from legacy hook!' })
      console.log('Legacy mutation result:', result)
    } catch (error) {
      console.error('Legacy mutation error:', error)
    }
  }
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-6">
          ...
          <div className="p-6 border rounded-lg">
            ...
            <div className="mb-4">
              <h3 className="text-lg font-medium mb-2">Legacy Health Check (GET)</h3>
              {legacyLoading && <p>Loading...</p>}
              {legacyError && <p className="text-red-500">Error: {legacyError.message}</p>}
              {legacyHealthData && (
                <pre className="bg-gray-100 p-2 rounded text-sm">
                  {JSON.stringify(legacyHealthData, null, 2)}
                </pre>
              )}
            </div>
            
            <div>
              <h3 className="text-lg font-medium mb-2">Legacy Test Mutation (POST)</h3>
              <Button onClick={handleLegacyMutation} variant="outline">
                Trigger Legacy Mutation
              </Button>
            </div>
          </div>
        </div>
